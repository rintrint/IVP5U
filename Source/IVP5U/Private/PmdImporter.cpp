...
namespace MMD4UE5
{
		...
		const uint8* BufferEnd)
	{
		...
		}
		{
			...
			{
				memcopySize =
					sizeof(char) * 20 + sizeof(uint16) * 2 + sizeof(uint8) * 1 + sizeof(uint16) * 1; // name --> IkParent;
		...
		}
		{
			...
			{
				memcopySize = 0 + sizeof(uint16) * 2 // Bone,TargetBone
					+ sizeof(uint8) * 1;			 // ChainLength
				FMemory::Memcpy(&ikList[i].Bone, Buffer, memcopySize);
				Buffer += memcopySize;

				memcopySize = 0 + sizeof(uint16) * 1 // Iterations
		...
		}
		{
			...
			{
				memcopySize = 0 + sizeof(char) * 20 + sizeof(int) * 1 + sizeof(uint8) * 1; // Name --> SkinType;
		...
		*/
		{
			...
			for (uint32 i = 0; i < PmxMaterialNum; i++)
			{
				...
				*/
				...
				tempTex.TexturePath = ConvertMMDSJISToFString(
				...
				if (tempShaPathStr.Equals(""))
				{
					...
					pmxMaterialPtr.SphereTextureIndex = -1;
				}
				else
				{
				...
				{ // Toonテクスチャ, テクスチャテーブルの参照Index
					pmxMaterialPtr.ToonTextureIndex = 0;
				}
				else
			...
			for (uint32 i = offsetBoneIndx; i < PmxBoneNum + offsetBoneIndx; i++)
			{
				...
				if (pmxBonePtr.Flag_IK == 1)
				{
					...
					if (tempPmdIKPtr)
					{
						...
						{
							pmxBonePtr.IKInfo.Link[j].BoneIndex = tempPmdIKPtr->ChainBoneIndexs[j] + offsetBoneIndx;

							FString tempChldBoneName = ConvertMMDSJISToFString((uint8*)&(boneList[tempPmdIKPtr->ChainBoneIndexs[j]].Name),
								sizeof(boneList[tempPmdIKPtr->ChainBoneIndexs[j]].Name));
							const char hiza[20] = "ひざ";
							FString tempHizaName = ConvertMMDSJISToFString((uint8*)&(hiza),
								sizeof(hiza));
		...
		}
		{
			...
			{
				if (0 < pmdMeshInfoPtr->skinList[i].SkinType && pmdMeshInfoPtr->skinList[i].SkinType < 4)
				{
